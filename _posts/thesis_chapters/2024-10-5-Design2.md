...
![Illustration 4.x - Larger game making activity system  ](./Pictures/At_dia_1_v4.png){width=95%}
...
The in-session audience and peer group appeared to drive some additional motivations and objectives. One of which put simply was play and social interactions between participants.
...
<!--
Introduction and draw on the documentation of the Mozilla community and select Phaser as a framework. -->
...
<!-- My expectation that participants would follow their own interests and as a facilitator I would help groups pull projects together. -->
...
### Description of contradictions related to docs

The previous section addresses changes to primary tools used to create game code and assets and response to the emerging tensions expereienced by participants in P1.
...
There were two main ways of approaching adding a new pattern.

As part of the vignette dialogue, to help a participant access support to add a new feature to the game, I outline the two main sources of documentation : "Here’s the tutorial and there’s the examples of code". In the course of the vignette Toby chose to access only a code example of the desired game behaviour. One to examine a code example which was built around the same code base. This would involve searching for new code constructs and notes in the code designed to orientate the learner, alerting them to the location and purpose of the relevant code. Other participants in this session, used the written tutorial which provided more descriptive, step-by-step guidance.  Other participants relied only on direct verbal help from myself or other participants (like Toby) to help them progress. In the following section I propose that participants  preferred approaches in accessing help may relate to a preferred game making style incorporating other elements of culture.
WHERE EXPLORED LATER?
...
MOVE SOME OF THE TEXT BELOW UP
...
My broad observations align with / EXTEND? the supported benefits outlined in research on the Use-Modify-Create (UMC)  [-@lytle_use_2019-1] and particularly the technique of starting with  half baked games [@grizioti_game_2018-1].
IT THIS RIGHT? OR DO THEY ADD ANYTHING?
...
#### Remixing, peer learning, and choice of starting template MOVE UP
...
This chapter has by focusing on first technical and then more social dimensions of the learning design, (re)surfaced the many barriers to achieving computational fluency via the text coding of a multi-media project. The next section provides a summary of the designs approach to try to address these tensions in relation to existing research, in particular UMC, half-baked games and  constructionist design principles.
