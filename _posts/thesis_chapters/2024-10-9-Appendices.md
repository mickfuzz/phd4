...
A patch is a quick fix to repair or to add new features to already existing code, supplied  to avoid a complete rewrite or more extensive process, and designed to easy for the end user to apply via a guided process.

For me the word patch contains relatable connotations of craft and physical repair of materials. The process of repair as a way to learn  skills useful in creation from first principles is applicable both to textiles and code skills development.  

Code patching within this study is similar to modern software patching processes which may be either automatic or rely on careful reading of forum posts and application in line with context of the original poster's (OP) code. Stack overflow is a culture in of itself with many explicit guidelines to request help and norming processes amongst learners. The 3M learning design contains processes which help participants to develop skills which move towards being able to post on stack overflow.
