...
The process of getting started in P1 took too long and from a motivation of an responsive lacked a guiding structure. To address confusion about how to being coding a game, I introduced a working code example
as a beginner template using the phaser library. Code
...
Starting templates are a common tool when learning new code frameworks particularly in web technologies. An example being the popular JS framework Bootstrap which provides a number of starting templates based on common design scenarios.

The template in this case followed the following structure:
- a beginning section out side of a funcition declaring variables
...
In P1, I found that as I worked with the learners to respond ot their requests to add in new elements to the game, I was able to to reference different parts of the template and thus build their familiarity with the code.
...
Despite building familarity with code, at this stage event simple game changes or adding new elements still required significant support.

In a redesign of the startign code template at the end of P1, following the UMC (Use - Modify - Create) model  [@lee_computational_2011], I added several following design techniques to the template to make quick modification of the game easier. Descriptions of three of these follow.
...
While the focus on this chapter is on design and designs decisions, the following sections include rough analysis of the impact of them. Fuller analysis follows in chapters five and six.  

MOVE THIS ANALYSIS
In line with this research, I posited that using a starting template would to increase speed and consistency of feedback from learner input, reduce coding syntax knowledge needed to progress, and thus reduce learner anxiety for novices.
This approach responds to the feedback from the participant that greater time allocated to hands-on use of tools before making design decisions would be welcome.
...
#### Quick Start Cards - MOVE TO LAST ONE?
...
While tempting to say that the use of a template is educational scaffolding and therefore not authentic to a work place setting environment, the use of starting templates is also a common work place tactic. For example the js libary boostrap comes with starter templates.

Glitch has many such starter templates aimed at professionals trying out a different code language or framework.
