...
In P1, I had encouraged the use of existing resources via google searches and searching developer resources the Phaser examples [^11]. The introduction of this process caused significant additional tensions and confusions. As with many developer created documentation, focused on demonstrating the code constructs in isolation rather than in the context of any particular game. In addition, any varience of

Initially, I try to counter this by creating one off documents withthe reelvant code which needed to be copied and pasted into their game[^12]. This seemed also unsatisfactory.

Instead I began to create code snippets which shared the struture of starter game template and added only the code needed for the additional requested functionality. .
