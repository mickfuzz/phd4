...
- gives an overview of the evolution of the key pedagogical features of the tools of learning design used in my research process
- (rewrite) draws on immediate analysis of participants reactions and contributions from journal notes, and participant interviews and analysis of changes in the learning resources created to present an exploration of tensions
- outlines 4 key conflicts using the terminology of Saninno in TADS
...
As explored in the literature review, there is a wide gap between instruction based tutorials and unguided project challenges. Even with the process driven scaffolding that a working knowledge of PBL approaches provides, the domain specific knowledge and practices can be a significant challenge for teachers. Thus project work which can place a high demand on the project facilitator/s. This chapter charts the evolution of tools and resources which occupy this middle ground between instruction and free choice creative approaches. Following chapters explore the impact and resulting pedagogies in more depth.
...
The following sections outline different scopes of activity to ground the reader in the overall activity and introduce later tensions between system elements. Each section follows a pattern: the activity is described in general, then examples of activity at that scope from the vignette above are described, ending with brief analysis or signposting to where the activity in explored in greater depth.
...
There are diverse end audiences for the games. The final target audience are students and staff in the Manchester Met Brooks building where their games are shared in arcade cabinets at the end of the course. An additional audience are friends and family who can be send the games to play online.

The more immediate audience for the evolving games are peer game makers during the making sessions. For example, in the vignette above, Toby begins the session playing his own game based on a shared template. During the session he invites many other group members to play his game. He initiates and responds to may conversations around the difficultly of the game he has designed. Analysis of this scope of activity is continued in chapter six.
...
The process of making a game was driven learners setting goals and requesting help to implement game features. The process involved a diversity of tools and resources depending on the feature chosen and the preferred approach of the learner.
...
a collection or menu of elements or design patterns (see chapter two and glossary) for the chosen game type of a platformer was in place. Toby browses the web page of a menu of game design patterns and selects moving enemies.
...
NOTE - Overall description and justification of this section.
...
The identification of different scopes of activity systems aids analysis of the complex and interwoven cultural, social and personal actions [@rogoff_observing_1995].

In this thesis, chapter six has a focus on cultural aspects of making a game together and chapter five explores social organisation around the activity of adding game design patterns chapter five. This chapter broadly aligns with a personal dimension of activity, specifically the use of tools and resources via the description of key contradiction in emerging game making activities. The next section begins to chart emerging tensions via a concise description of the evolution of tool use in different phases of the study.
...
In previous sessions, members of this family were able to engage in planing via sketching on paper and in creating pixel art. However in later sessions, they were reliant on others to implement code changes for their imagined game due to way the improvised group roles had emerged.
...
This aligns with research in this area explored in the LR which conceive coding practices as alien to certain groups in particular, and stress the import ance of x.
...
NOTE - There are two sides of contradiction, one is the clash between curriculum and choice, the other is in my hesitancy introducing activities using the new tools of the maps due to potential overload and not enough time.
...
My progress in this study would have been more direct if I had not attempted to resolve the conflict between the informal choice and curriculum alignment. Ultimately, despite developing resources, I did not introduce the resources consistently with the learners of this study, instead prioritising time spent in on-going hands on experimentation, peer learning and playful activities.
...
**Nature of conflict - unfamiliarity with tools- complexity of processes leading to frustration**
...
In Scatch there are tabs which host simplified graphical and audio studios. Design decisions prioritise ease of use and the ability to record audio right into the program. The self-contained of scratch has this reduces possibility for compounding errors and complexity caused by file formats, migration of data. However, to justify this choice, I observed a palpable a sense of achievement when participants succeeded in seeing and hearing their creations in their game after making the final changes in code. The reward seemed to be greater due to the difficulty involved.
...
#### Design affordances included in the game template

This section draws on HCI theory and in particular the concept of affordances.

While the focus on this chapter is on design and designs decisions, the following sections include brief analysis of the impact of them. BECAUSE
Fuller analysis follows in chapters five and six.

In the design of the template and in testing at the end of P1, I began to identify simple changes to the code that could provoke a high impact on the game experiences. The next sections describe and analyse these changes using affordance theory.
...
These include changing gravity, altering the player speed,

I brought certain variables that had high impact on the game experiences to the very start of the code.

EXAMPLE VIA ILLUSTRATION

The starting template began with the game in a broken state. Participants needed to change the code to allow effective player movement. This thus forced all participants to alter at least one of the key variables were highlighted at the very start of the game code.

Following this first change however behaviour varied greatly. While some participants engaged with extensive experimentation to find a player movement feel that seemed just right, others, mostly adults or younger participants, were much less concerned with this aspect of game play, despite sometime frustrating resulting player movement.
...
In early trial, I had relied on participant experimentation, direct instruction to individuals / pairs (via pointing), and resultant peer learning happening. I relied on small groups initially.  

For larger groups this was not optimal, leaving some learners confused and unsupported. At the end of P1 I tasked trainee computing teachers in my faculty created printable resources that highlighted particular key lines of code and variables (see figure 4.x below) and illustrated the relevant code changes needed to create big changes in the game.
...
The initial use of a template was an intuitive response based on my own experience of teaching technology. However, as the template evolved, I adapted my design with an aim to increase speed and consistency of feedback from learner input, reduce coding syntax knowledge needed to progress, and thus reduce learner anxiety for novices.

While the simple changes to the game dynamics and patterns embedded in the starting template helped motivation and engagement, some participants soon began to ask how to make additions to the game.

This point marks the pivot to the create _phase_ on the UMC model. Thus the shift from P1 to P2 involved a much more careful and structured experience for participants in U and M stages.

While I do not have comparative video and interview evidence between P1 and P2, journal notes of observations of engagement support this interpretation and alignment with existing research. Broad, uncontroversial claims can be made. For example, new participants in P2 all maintained a working game thus able to share during each session. 
...
A key conflict in the overall activity system was the inability for participants to make progress on their game due to lack of working processes and domain knowledge. To address this, in P1 I tried undertaking class instruction of key coding concepts, e.g. logic statements, that I judged to be relevant to the whole group. Despite trying to limit the time spent on instruction, linking to their experiences and making the process informal, my journal notes reflecting on these interventions were unfavourable.

As outlined above, my model emerged with similarities t the UMC approach. While the starter template and quick activities served to address the _use_ and _modify_ stages, the _create_ stage required additional documentation as there were not enough facilitators to help participants with their individual choices of what to add to the game. This section outlines the evolutions and design choices informing the different forms of supporting documentation used.  
...
EDIT DOWN - WHAT IS THE KEY PURPOSE OF THIS BIT?
...
This section has described the process of categorising  different themes the game pattern collection.  Similarly this outcome of theming the patterns by function and  difficulty to help build community game making activity and knowledge aligns with the work by Holopainen and colleagues on the use of pattern collections to help structure game making for adults [@holopainen2007teaching]. This theme is continued in the next chapter which explores the impact on users of GDPs.  
