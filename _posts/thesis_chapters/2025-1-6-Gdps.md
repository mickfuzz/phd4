...
Waite and colleagues propose that abstraction should be taught to pupils [@waite_abstraction_2016].

Waite and colleagues also propose the utility of awareness of LOA in terms of learners knowing at what level of abstraction they are operating, although the authors call for further to confirm findings  [@waite_abstraction_2018-1].

As a challenge these findings, My own notes and reflections in appendix.learning map cautioned me against such shifts in perspectives for fear of interrupting the flow of participant making. I had a reluctance to shift learners away from the practical implementation of repeated game design patterns to focus on more abstract, de-contextualised conceptions of the knowledge. My concern hinged on the potential disorientation of imposed shifts of focus. There were already shifting from the larger goal of making an engaging game for a real and imagined audience to a narrower goal of implementing a game design pattern and the technical actions and operations needed to complete it. This chapter has outlined the potential of GDP concepts, especially used within playtesting, to support these participant shifts in perspective. To then add an additional goal of then connecting to abstract curricular concepts seemed counter productive to the overall aim of acculturation to game making tools and processes and developing an inclusive idioculture.


As such this research has only more theoretical propositions rather than participant data. However, my reflections do support a proposition that there is  utility is that facilitators to be aware of the level of abstraction both that present in different elements of project learning design and that of the concepts that learners are working with. Different purposes and contexts of learning may require different interpretations, for example greater awareness of abstract concepts for reasons of assessment.

According to [@barendregt_intermediate-level_2018]  when choosing design patterns, choosing a suitable the level of abstraction is recommended. The evidence and examples surfaced in this paragraph support a view of GDPs, as an productive intermediate-level framework and contributes a series of grounded pedagogical instances via examples. It also proposes GDPs as potentially more productive and accessible framework in terms of pedagogical uses than that of computational thinking when making games.

**integrate better**

Beyond changing perspectives in participants facilitated by GDPs outline above, addressing the impact of scaffolding awareness of and reflection on shifts in levels of abstraction in the coding and design process, may be tangential to this study, it is a potentially productive strand of future investigation.  

Levels of abstraction and shifting perspectives also exist in formal analysis of game design frameworks [Salen, and MDA].
In the theoretical underpinning, Aesthetics are the first experience of the user, and the designer most closely aligned with mechanics.

Could awareness of this their shifts in perspective in relation to this MDA framework be helpful to novices.
...
Other questions concerning LOA for participants / facilitators.

This learnign design could be a good base to use to answer those questions.
...
Thus this chapter returns to celebrate with Papert and Turkle's the process of concrete tinkering, but also to suggest that it is potentially augmented, in terms of wider accessibility, with the use of design patterns as a germ cell concept. This approach is flexible and potentially response whilst still retaining a unifying pedagogical approach.

In many of the outlined uses of GDPs in chapter we can see processes at play that help bridge shifts in goals between activity systems and navigation between one sub-goal and the nexts.  
Any other analysis on the part of the participant can wait until the process becomes more familiar.
...
**Some of the following processes stand out for particular promise.** LINK TO THE SUGGESTIONS FOR PractitionerS.
