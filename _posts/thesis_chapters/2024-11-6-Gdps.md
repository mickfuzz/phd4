...
Beyond providing the reader with an illustrative example of partner work in game coding, this commentary has a primary focus on surfacing uses of GDPs as a meditational strategy, which are then explored using wider data in the rest of the chapter.

The child(c) Olivia uses the name of game design pattern they are working in _Keys and Doors_ is repeated in a sing song voice . She also chivvies her parent(p) Suzanna to get started on it (Key. in Vignette 2.).

There is evidence that the Olivia(c) is building a connection to the game making pattern in (Key. in Vignette 2.).
