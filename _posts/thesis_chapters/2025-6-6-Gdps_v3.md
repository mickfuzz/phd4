...
Vignette 2 follows the parent Susanna's use of a tutorial to address the goal of implementing her child's chosen GDPs. Susanna, is limited in her programming skills and avoids seeking facilitator or peer help [^fh]. As such, she  independently access written step-by-step instructions as a preferred working approach. Susanna leads the process implementing code changes of patterns chosen by her daughter [^si]. Susanna leverages the concept of the key and door pattern to access relevant documentation.


In the Vignette 1 extract above, Toby uses the graphical menu of patterns as a starting point to help him to choose, find and implement a GDP feature using supporting documentation. He takes a more direct approach and does not access the step by step instructions.

Later he is able to assist others in adding the GDPs to their games [^rc2].

THIS IS NOT NAVIGATION - BUT IS RELEVANT CLOSE COUPLING - MY INPUT AS FACILITATOR

THE DIFFICULTY STRUCTURING

This choice appears to have a positive impact on affect.
...
[^si]: In this case, her daughter's choice has been influenced by social interactions with the group.  The concept of key and door (a pattern where the player needed to collect a key before progressing through a door in the level) had been discussed as a group previously. See Appendix D.3.map
