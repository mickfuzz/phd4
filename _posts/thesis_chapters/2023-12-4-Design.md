...
Many software projects aimed at for novices use visual coding (block coding) approaches to reduce complexities of code use. This process reduced possibilities for syntax errors and often provides a limited set of blocks to reduce complexitiy. While this approach not explored here for reasons previously outlined, there are some features of the glitch code playround which help. For example, the environment can detect the file type as javascript from the file extension and then uses a static analysis tool (linter) scan the code for signs of inconsistencies of code and syntax errors and highlight them [@tomasdottir_why_2017].

In glitch such code syntax errors (GL) are highlighted via a red dot provided a quick visual indication of where the error occurred. In addition if the user hovered over the dot would give an error message.

On several occasions in analysis of pair interactions, I observed the non-coding peer notice and point out the red dot, thus preventing further errors. The process worked especially well when altering existing templated code, a process which is explored in the next section.



#### Shared community and remix ability

The ability to easily remix community projects, supports the process of UMC. Specifically, see inside offer and remix offer when viewing scratch projects.

While my design used Glitch community, which had this capacity, the use was much broader, and Glitch less useful. Thus my design was lacking.  
...
#### Shielding complexity of environment and access
...
A game code project using the chosen framework phaser will consisted of several interlinked files of javascript, HTML and CSS and image files. The process is relatively complex both semantically and practically to enact from first principles.

I therefore decided to shield learners were shielded from this complexity by using a starting template with base version of the files already created with the links already coded.  

This decision mirrors the design choice in Scratch and Code.org sprite and game labs to create a default canvas screen size of the project. It is already present and visible on the creation of each new project and the user has no choice over the settings. This preset design decision align with a constructionist design principle "be careful about the use of black boxes" FIND SOURCE.

A black box here refers to the process of hiding away aspects or functionality of the code or processes.  The choice here to have a default for the canvas size is to reduce initial barriers to getting started, so the user is not presented with unfamiliar options at the start of the making process.

In my design, I have made similar decisions to shield participants from complexity to reduce cognitive load and alienation from unfamiliar language. However, because of my choice to use authentic tools, while the complexity remains in code view, I decided to use a starter template to de-prioritise the impact of the underlying code complexity on the participants experience.
...
The choice to use a starter template was driven by early experience in the experimental period of the design.
<!-- In participant feedback for P1, one parent indicated that it took too long before in the planning stage and called for more hands on play and use of the tools of production before being called on to make creative decisions. The parent likened this to an arts studio approach. -->
