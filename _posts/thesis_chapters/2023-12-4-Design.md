...
**Addressing access (and environment) issues**

MOVE TO LR.
The process of running web code can be complicated by setting up a server environment locally to test and then finding and uploading to a web server. Code playground allow users to circumvent such problems. While originally designed more for problem solving, a service more recent code playgrounds provide is to act as a more permanent host for web projects.  

As explored in the LT, Code playgrounds address issues of complexity of web coding part by providing a self contained server. However for novice  users the web location still also offers friction in terms of digital lieracy issues of logging in, remembering passwords, linking to email accounts for password reminders. To circumvent these issues in P2 and P3 all participants and facilitators used a shared account for code playground the graphics editor.
...
Using a code playground removed the process of setting up a server for web development, and the delay and complexity of transferring files to the server via FTP/SSH processes.
...
The examples above align with constructionist design principles explored in the LR

In particular  "Be careful about the use of black boxes" FIND SOURCE. This refers to the process of hiding away aspects or functionality of the code or processes. For example, in Scratch the process of setting up a canvas is preset. It is already present and visible on the creation of each new project and the user has no choice over the settings.
