...
In my design, I have made similar decisions to shield participants from complexity to reduce cognitive load and alienation from unfamiliar language.

A game code project using the chosen framework phaser will consisted of several interlinked files of javascript, HTML and CSS and image files. The process is relatively complex both semantically and practically to enact from first principles. I therefore decided to shield learners were shielded from this complexity by using a starting template with base version of the files already created with the links already coded.

However, because of my choice to use authentic tools, while the complexity remains in code view, I decided to use a starter template to de-prioritise the impact of the underlying code complexity on the participants experience.
