...
Some studies outline a generalised use of PBL and design stages to structure game making  revealing underlying characteristics which clearly align with a project based learning (PBL) approaches and provide structural support [@kafai_constructionist_2015; @denner_does_2019]. For example, the end goal of making a game provides an readily understandable driving challenge which can be shared publicly via a playable game to an authentic audience, either in person or online [@pitot_establishing_2024]. Simmons et al. [-@simmons_using_2012] briefly outline the use of a five stage design process to help structure the planning process of students. Similarly returning to the example of Globaloria programme [@reynolds_formal_2013-1], researchers found the the intrinsic motivation of game making sustained engagement through the design stages increasing the likelihood of participants to progress to test and subsequently revise their game creations.

However, most studies cited by Denner et al. as illustrating a design-build-test approach show significant limitation of details either the the students design process or suggested scaffolding of that process communicated to teachers or facilitators [@wang_relationships_2011; @wang_learning_2010-1; @robertson_influence_2013;  @ke_implementation_2014]. This may be in part due to limitations of the publication format, but may also reflect genuine limitation. For example, the online supporting resources of Adventure Author research by Robertson focuses primarily on software use rather than facilitating design process [^9]. In addition to these more general uses of PBL support, more structural and specific forms of scaffolding exist in the domain of CGD&P as distinct pedagogies which are addressed directly in the following sections.
...
### Use-Modify-Create (UMC) & Half Baked games

The 'Use-Modify-Create'(UMC) approach proposed by Lee and colleagues [-@lee_computational_2011] is particularly promising to counter issues of user anxiety and demotivation surrounding the difficulty of coding games. UMC evolved from research involving the use of game making and robotics to support computational thinking [@denner_computer_2012; @denner_using_2014; @werner_pair_2013; @werner_children_2014]. The model advocates the remixing of existing games to act as a scaffold to build the competence of novice coders. Learners are guided to progress in the complexity of their modifications, thus becoming increasing proficient in the recognition and use of computational concepts and structures [-@lee_computational_2011]. In the _Use_ stage, coders build a familiarity with coding interfaces, code structures and syntax. In the _Modify_ stage learners progress to working on existing projects created by others, thus deepening their knowledge of coding concepts and practices by altering existing code to achieve their own aims. _Create:_ After novice coders become more familiar with patterns of code design in use in the modify stage, they can progress to replicate such patterns in other code projects that they create from scratch.

UMC can produce increased level of engagement through reducing technical barriers to participation, and affording greater sense of learner's ownership over end project through greater choice over the final outcome. A study involving five hundred 9 to 14 year-olds found that the UMC approach can balance tension between providing a structured approach as well as supporting  student-led exploration [@franklin_analysis_2020]. The researchers also found that the students enjoyed the UMC approach, particularly the greater choice and agency in the process. Other research compared UMC with a starting-from-scratch approach and found higher student engagement for those in the UMC group [@lytle_use_2019], due to students using UMC having greater time manipulating code, in a way that allowed them to add their own personal touches thus increasing sense of ownership over the code projects. These results align with constructionist research on the value of  choice over the design of created public code products has the potential to motivate ad sustain programming activity [@kafai_constructionism_1996]. However, this choice can potential provoke both the potential for diversion of students away from target areas of subject exploration, and facilitator stress in supporting diverse activity. Noss and Hoyles [@hoyles_pedagogy_1992] dub this first tension _the play paradox_. In order to address the second tension between choice and potential teacher stress caused by the open ended nature of the "Create" part of the model, Lytle and colleagues suggest a list of extensions to choose from, swapping create for choose [@lytle_use_2019-1].
...
Half-baked games, proposed by Kynigos and colleagues [-@kynigos_half-baked_2007; -@kynigos_children_2018], are games that are intentionally unfinished and/or deficient in a way which motivates learners to alter the design or code and improve it. The concept builds on Papert's work on Microworlds [-@harel_constructionism_1991; -@papert_mindstorms_1980]. Like Microworlds, half-baked games are designed to encourage malleability in directions which the learner may find interesting [@kynigos_children_2018], and which the learning designer intends to be educationally productive. To do this the designer of the half-baked game makes complex decisions which highlight certain affordances of the game in a way which encourages the exploration of key concepts [^10]. Kynigos and Yiannoutsou  [-@kynigos_children_2018, p.2] conducted a study which provided "tools affording them (13-15 year olds) with the role of game hackers" to alter existing game code. The researchers identified progression in the complexity of coding skills used by learners over the intervention from pattern recognition associated with reading of code to more advanced examples including creating abstracted structures and sequencing their own algorithms. In previous work Kynigos [@kynigos_half-baked_2007, p. 336] described the possibilities of half-baked artefacts to build learner dialogue around the problems at hand as as 'a communicational tool to shape a common language within the community', suggesting an alignment with social and cultural pedagogical approaches.
...
Work of Sentence and Waite culminating in a recent report on use of teaching strategies within a UK computing education context [@waite_teaching_2021] offer a range of relevant pedagogical approaches including: levels of abstraction, semantic profiles & PRIMM. The concept of levels of abstraction (LOA) can facilitate both teachers and students to help them develop a hierarchical understanding of elements of  the coding process [@statter_teaching_2016; @waite_abstraction_2016; @waite_abstraction_2018]. While the term abstraction has varied interpretations within the field of computer science [@hazzan_reducing_2002], here it reflects a spectrum between overarching concepts and the concrete results (see Table 2.1).
...
Later work by Waite and Sentance [@sentance_primm_2017] draws on the ethos behind  UMC, LOA, and semantic profiles [@macnaught_jointly_2013] to create an applied pedagogy (PRIMM) for computing education. The motivation is address the tension between exploration-based approaches and those proposing greater instruction and guidance[-@sentance_teaching_2019, p. 5]. The approach is also a response to calls to align computing education with sociocultural approaches [@tenenberg_out_2014], drawing on concepts, of mediation and ZPD [@sentance_teaching_2019, p. 2]. PRIMM (signifying Predict, Run, Investigate, Modify and Make) extends the UMC model, by adding additional stages, thus advocating that learners should begin by predicting the outcome of a given concrete code example before check their predictions against the results of running the code. Subsequently with guidance learners should investigate potential code changes before modifying the code provided. The final, _Make_ stage (aligned with _Create_ in UMC) envisages students creating programs or larger elements of code structures from scratch.

<!-- **Data from research, response of teachers , application and take up of SW and PRIMM** -->

The adoption and discussion of PRIMM in learning contexts has been significant [@martin_extending_2020; @parry_investigating_2020; @ofsted_research_2022; @barefoot_computing_crystal_nodate], perhaps due to its concrete applicability in classroom settings and the inclusion in a range of pedagogies promoted by the National Centre for Computer Education (NCCE) via resources on computing pedagogies [@ncce_using_2020]. However, while nothing within the PRIMM and LOA pedagogies makes them incompatible with non-formal environments, little or no relevant research has been carried out in non-school settings. Sentence et al. [@sentance_teaching_2019] acknowledge other limitations of the research proposing future work addressing the areas of co-production of possibilities for differentiation together with teachers based on sociocultural approaches, and more careful alignment between the planning of sessions using LOA concepts and delivery using the PRIMM approach.
...
The review of CGD&P carried out by Denner and colleagues [-@denner_does_2019] identified The strand of social interaction as a third pedagogical strand present in 35 of 68 studies. The review, as with Earp's [-@earp_game_2015] earlier review, notes  limitations in scope of the social and collaborative interventions, restricted almost entirely to pair programming approaches and forms of general feedback on game projects.
...
Pair programming is a common industry practice which is also applied in educational contexts [@hanks_pair_2011]. Pair programming groups students in pairs and prescribes two coding roles. One student undertakes hands-on coding while the other is free to think about more the abstract design of the overall program. A benefit of pair programming is to increase coding confidence as students build their experience of the different roles involved in coding. To help novice coders teachers should model and break down the processes involved. Werner and colleagues [@werner_pair_2009; @denner_computer_2007] investigate pair programming as way to address gender gap and extending research on collaborative problem solving in computer coding. They cite research challenging the gender aspect of bricolage / abstract duality, but propose a need for more research on programming styles and strategies [@denner_computer_2007] Their own research underlies that while pair programming is an inclusive strategy beneficial to all but in particular to narrow participation gaps due to gender and socio-economic factors [@werner_pair_2009, p.31]. In Denner's research, pair programming involved social learning elements and can model a greater choices for students in they way they solve problems and opportunities to develop identities. The process of building an identity in a community with the help of peers is key to a socio-cultural understanding of how learners pick up coding in a classroom (or other settings).


The value of these approaches is clearly not limited to pair work and can be applied to collaborative groups of larger sizes. For example, the work of Werner and Denner described above draws on existing research on collaborative social reality or joint problem solving space to scaffold the process of ideation [@omalley_construction_1995] and the role of friendly relations to develops these productive and sustained interactions [@mcdowell_pair_2006]. Additionally, Waite and Sentance [-@waite_teaching_2021] explore the possibilities of peer instruction in a computing education context both as a specific pedagogy [^11] and as a more general collaborative approaches to learning programming .

Popat and Starkey [@popat_learning_2019-1] note that collaboration and sharing of code as a way to help peers often emerges as a helpful bi-product rather than a planned element of learning designs.  Earp and colleagues [-@earp_learner_2013] note the surprising deficit in studies which explore collaboration as a process and learning outcome given that the diverse roles in game making suit such activity. The existing gaps are also notable given the development of learner identity within a collaborative culture are considered fundamental for exploratory project-based approaches [@kolodner_problem-based_2003]
...
Coding clubs are addressed above via the constructionist strand of research drawing on early legacy of the computer clubhouse work [@peppler_computer_2009]. In a UK context, despite extensive volunteer led activity only limited research exists on three related coding club projects, Code Club, Coder Dojo, and Raspberry Jam [^12]. Existing  research does covers the use of Code Club resources by volunteers [@aivaloglou_how_2019], general approaches to facilitation, and a lack of an overall conceptual model at Coder Dojos [@alsheaibi_teaching_2020]. One strategy identified to help focus the activities of participants is to encourage entry in computing and engineering competitions [@aivaloglou_how_2019].  

From 2010 to the present day there have been a variety of coordinated online CGD&P competitions delivered by either entrepreneurial organisations like the Globey award from Globaloria, from foundations  or public bodies (e.g. Games for Change and the STEM Video Game Design Challenge sponsored by the White House in the US) [@kafai_social_2013].  Game projects also form a significant contribution to the Collab challenge which exists as as part of the Scratch online community, [@kafai_motivating_2014; @kafai2011collaboration]. In the UK context the Coolest Project competition works in partnership with Coder Dojos and Code Clubs. Research by Quinlan and colleagues [@quinlan_how_2018; @quinlan2020ideas] on the Coolest Project explores themes emerging from case study interviews with participants. The research identified different entry points for project engagement, some focusing initially on technology, some on the idea/problem and some on skills involved. The authors also identify broad benefits of such competitions which are echoed in wider research including: the inspirational and motivational value of seeing the work of others [@kafai_what_2012; @kafai_motivating_2014], development of varied forms of confidence, the development of team work skills and collaborative problem solving, and the valuable influence of adults in the process in particular to help build connections to communities of work and expertise and the logistical support of parents [@quinlan_how_2018]. The final consideration here also forms the basis of a critique of competition regarding inequality of access [@thumlert2018learning]. A further concerns is that the values underlying computing competitions are largely embraced uncritically  [@thumlert2018learning], a concern inherited from the wider maker movement [@vossoughi_making_2016].
...
While this chapter has covered many promising pedagogies and studies involving strategies relevant to enhance CGD&P, a common thread of limitations addresses lack of alignment with theoretical perspectives addressing social and cultural aspects of the learning process. To address this, this section examines a collection of educational partnerships called 5th Dimensions (5thD) which took place in technology-rich, non-formal settings, often in after-school clubs within lower income communities [@cole_design-based_2016]. 5thD interventions had a primary research goal to develop sociocultural  understandings of learning processes. Initial iterations, led by Michael Cole, and took place around San Diego University with the support of volunteers, equipment and technical support to design and help deliver a creative series of computer-based, playful activities. Cole [@cole2009designing] describes the two contextual motivations driving the design of the 5thD activity as the ongoing need for provision of activities in after-school settings, and as a process  to provide relevant experiences to undergraduate students, allowing them  to connect academic understandings of child development to real life experience. The choice to use novel computer communication technology of games and email addressed both a chance to address reading deficits [@cole2009designing; @cole2014designing], and potential alienation of women and girls from STEM subjects [@cole1987contextual]. 5thD researchers analysed the resulting programme as a joint activity between partners with mutual but differing aims.  The intervention, employed a narrative fiction of a wizard with whom young people interacted via email. This motivated and orientated young people to undertake both digital and/or play-based activities that also developed their written and computer literacy [@brown_cultural_2008]. The ongoing nature of the project resulted in regular young attendees helping new cohorts of student volunteers to understand how the ongoing activity functioned on technical and social levels in a way that developed a sense of expertise for young people as well as important understandings for student helpers about the mutual learning processes at play  [@cole2009designing].
...
[^20]: An example of a worksheet created is availble from the supporting resource site for the Scalable Game Design programme.  https://web.archive.org/web/20220331110501/https://wiki.computationalthinkingfoundation.org/wiki/images/5/5b/ACO_Frogger_Student_v1.0.pdf
...
[^22]: RQ1: What contradictions emerged during participation in CGD&P activities and how were they addressed via an innovative pedagogy?

[^23]: RQ1: What contradictions emerged during participation in CGD&P activities and how were they addressed via an innovative pedagogy?
