...
In P2 starting template graphics consisted of colour blocks, a design choice aligned with a half-baked game approach inviting learners to develop game characters from a clean slate.  

The size of the colour block in the starter template were  32 x 32 to align with the default project size of Piskel (the graphical pixel art editor) to reduce technical friction when designing and swapping in new graphical resources.  This standardisation also helped resolve a complications encountered in P1 due to differing sizes of sprites created using different graphical tools.  

DROP? I had previously set the dimensions of the matrix above based on this size to create a resulting game size of 550, 400, in line with a retro game style. 
