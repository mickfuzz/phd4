...
Despite the efforts to simplify the process, the process of swapping in new graphical assets is (outlined in Appendix 5.tech) still required a series of potentially tricky operations made more complicated by the distributed nature of the process. While some novice code authoring tools adopt a self-contained to audio and graphical asset creation and by providing in-built authoring tools and a library of assets, the code playground Glitch provided neither, thus requiring the use of external asset creation tools.
...
The in-tool scaffolding within primary tool outlined in the previous section aligns with microworld concepts  of affordances guiding learners towards the exploration of key concepts and reduced reliance on facilitator instruction [@laurillard2020significance], an alignment explored in more detail in this chapters' discussion. However, these design changes did not aid participants wanting to implement new features or add code structures into their game, as aligned with the _Create_ stage of UMC.

However, these design changes did not aid participants who wanted to create new features or code structures in their game. In P1 and early in P2 this caused delays and participant frustration, as although I could work with people directly to help them I had limited time with each participant or pair. Additionally, my responsive approach to instruction and documentation in P1 lacked coherence in approach and lacked a system for users to navigate the resources created.
...
In the development period (D1) after P1 reflection on the tension between learner agency (in particular the role of divergent participant learning paths) and clarity of instruction and support reoccurs consistently in my notes at this time.
...
The design and implementation of supporting resources continued to evolve at a frantic pace at the start of P2 and I finally created three main sources of documentation: quick start cards, written instructions and code snippets which are outline in the following section.
...
This following summative narrative outlines of the use of GDPs, which were introduced as part of several guiding activities. These include:

In P1 GDPs identified via playing games as part of an activity adapted from Moveable Game Jams
And used in list of features used as a mediating planning tool.

In P2 GDPs were incorporated as affordances into a starter game template in line with Microworld pedagogical principles
...
The use of  design decisions aligned with the principles above had a knock on effect on the increase use of social coding processes.

I prose in particular that the use of a template and a collection of GDPs via a guided approaches had an accelerated effect on a specific set of coding fluency practices.

This instrumental proficiency was required before the rich set of  interaction based on peer learning, and other social coding processed could emerge in playtesting practices.

Thus instrumental agency can be seen as the base of a triangular hierarchy of agency  One level may  needs to be built on the other.
...
Agency in this learning process was present via the input of participants in the changing design of the program. At times through direct requests, at times through my responses to behaviours demonstrating their needs and motivations. Playtesting as a process and the use of GDPs in varied ways by participants was noticed and I added more affordances designed to facilitate these aspects. Affordances here as a form of delegated agency.
...
THE PROCESS WAS OPEN, RISKY, BUT VERY PRODUCTIVE.
I was lucky to have a group in P1 as tenacious as that, and I also benefited by building on previous work.

MOVE SOMEWHERE OR ADD SOMETHING ELSE TO CELEBRATE THIS PROCESS
