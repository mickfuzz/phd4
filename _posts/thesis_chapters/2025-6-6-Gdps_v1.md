...
This discussion section discusses the complexity of the expanded object of activity and the diverse mediational uses of GDPs using a activity theory as a framework. It summarises the different uses of GDPs in a table and notes the limitations of 3GAT to represent such diversity of mediational strategies.  

This discussion begins to address the challenge of communicating the diverse mediational strategies beyond the list in the table, suggesting SDBE's concept of repertoires (and signposting a more accessible and practical conceptual model).
...
A fuller table summarising this chapter's interpretation of the diverse roles of GDPs in the game-making activity using CHAT concepts and linking to supporting evidence is included in the Appendix WHICH ONE.

The table lists example behaviours and notes vignettes or appendices where the behaviour can be found in situ with more detailed analysis.

REVISIT AND CLARIFY.
GDPS, are used not just mediation as a tool, but also as a concept, so much so that it merits it's own activity system.

Given that both the wider object of activity represented at the community game making process and the narrower object of each game instance, can be interpreted as a shared object for expanded, interacting processes of activity.
AND THAT THEY ARE COMPLEX OBJECTS. Attempts to represent the variety of uses of GDPs within the technique of triangular visual representations would be problematic.

In particular, the complexity in terms of the role of GDPs here which act sometimes as mediational strategies and at other times as motivational components of the object at hand would be difficult to communicate. Similarly, representation of the interplay of mediational strategies of tool use with the community level of activity and activity wider systems would also be challenging and unwieldy. As Engeström and Saninno [@engestrom_mediated_2021] note in discussing challenges of representing new generations of AT, you can't keep adding triangles.

SO WHAT?
SIGNPOSTING TO CHAPTER 7 REPRESENTATION?
...
THIS ISN'T CLEAR -
Given the complexity of the object being worked on in this research, and complex nature of the emerging activity, [@biffi_chasing_2017-1; @de_freitas_classroom_2012; @cormier2008rhizomatic], it is not surprising that a diversity of conceptualisations of what experience or attribute is being developed in participant in CGD&P or similar digital making.

The discussion in Chapter 5 outlined an extensive reflection my motivations in the changing design, a process starting from a point of facilitating a collaborative learning environment. While collaboration was certainly important, it did not summarise my motivations well.
...
Extending Basawapatna and colleagues [-[@basawapatna_zones_2013]] approach to support  flow SOCIAL FLOW? (see Chapter 2 - DROPPED) via just in time documentation aligned with participants goal to support participant engagement, adding greater specificity and choice over which game patterns participants implement. This adaptation help greater alignment with research on the interdependence of flow experience and possibilities for social identity development [@mao_optimal_2016].
